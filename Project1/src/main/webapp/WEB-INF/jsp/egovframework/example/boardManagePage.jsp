...
<link rel="stylesheet" href="<c:url value="/css/egovframework/admin/boardManagePage.css"/>" />
		...
		<div class="main_board_content">
			...
			<div class="page_navi_container">
				<c:if test="${pageInfo.showPrev}">
					<span><a class="page_navi" href="<c:url value="/admin/board.do?page=${pageInfo.beginPage-1}&pageSize=${pageInfo.pageSize}&condition=${pageInfo.condition}&keyword=${pageInfo.keyword}"/>">&lt;</a></span>
				</c:if>
				<c:forEach var="i" begin="${pageInfo.beginPage}" end="${pageInfo.endPage}" step="1">
					...
					<c:if test="${i != pageInfo.page}">
						<span><a class="page_navi" href="<c:url value="/admin/board.do?page=${i}&pageSize=${pageInfo.pageSize}&condition=${pageInfo.condition}&keyword=${pageInfo.keyword}"/>"> ${i} </a></span>
					</c:if>
				</c:forEach>
				<c:if test="${pageInfo.showNext}">
					<span><a class="page_navi" href="<c:url value="/admin/board.do?page=${pageInfo.endPage+1}&pageSize=${pageInfo.pageSize}&condition=${pageInfo.condition}&keyword=${pageInfo.keyword}"/>">&gt;</a></span>
				</c:if>
			</div>
...
<script>
	...
	const onDeleteBoardBtn = (bno) => {
		location.href = "<c:url value="/admin/deleteBoard.do?bno="/>" + bno;
	}
	...
	searchBtn.addEventListener("click", () => {
		console.log("click");
		form.method = "GET";
		form.action = "<c:url value="/admin/board.do"/>";
		form.submit();
	});
	
	writeBtn.addEventListener("click", () => {
		location.href = "<c:url value="/admin/writeBoard.do"/>";
	})
	...
	document.querySelectorAll(".table_content_tr").forEach((element) => {
		...
		element.addEventListener("click", () => {
			...
			location.href = "<c:url value="/admin/readBoard.do"/>?bno=" + element.querySelector(".table_bno").querySelector("#bno_number").innerHTML;
		})
