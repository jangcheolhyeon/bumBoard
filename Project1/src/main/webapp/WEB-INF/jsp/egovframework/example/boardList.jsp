	...
	<div class="main_div">
		...
		<div class="main_board_parent">		
				...
				<div class="search_container">				
					<form id="form" class="form">
						...
						<select name="category">
							<option value="All" ${param.category == "All" ? 'selected' : ''}>전체</option>
							<option value="Free" ${param.category == "Free" ? 'selected' : ''}>자유</option>
							<option value="Qna" ${param.category == "Qna" ? 'selected' : ''}>QNA</option>
							<option value="Notification" ${param.category == "Notification" ? 'selected' : '' }>공지</option>
							
						</select>
						
						<select name="condition">
							<option value="A" ${param.condition == "A" ? 'selected' : ''}>제목 + 내용</option>
							<option value="W" ${param.condition == "W" ? 'selected' : ''}>작성자</option>
			...
			<div class="show_container">
				<div class="board_count_container">
					<div class="board_admin_write_container">
						<span>공지사항 감추기</span>
						<input type="checkbox" id="hideNotice" ${param.hideNotice == null || param.hideNotice == false ? "" : "checked"} >
					</div>
					
				 	<select name="pageSize" id="boardCnt">
						<option value="10" ${param.pageSize == "10" || param.pageSize == null ? "selected" : ""}>10개씩 보기</option>
						<option value="30" ${param.pageSize == "30" ? "selected" : ""}>30개씩 보기</option>
						<option value="50" ${param.pageSize == "50" ? "selected" : ""}>50개씩 보기</option>					
					</select>
				</div>
			...
			<div class="page_navi_container">
				...
				<c:if test="${pageInfo.showPrev}">
					<span><a class="page_navi" href="<c:url value="/board.do?page=${pageInfo.beginPage-1}&pageSize=${pageInfo.pageSize}&condition=${pageInfo.condition}&keyword=${pageInfo.keyword}&hideNotice=${empty param.hideNotice ? false : param.hideNotice}"/>">&lt;</a></span>
				</c:if>
				
				<c:forEach var="i" begin="${pageInfo.beginPage}" end="${pageInfo.endPage}" step="1">
					<c:if test="${i == pageInfo.page || pageInfo.page == null}">
						<span><a class="page_navi current_page" href="<c:url value="/board.do?page=${i}&pageSize=${pageInfo.pageSize}&condition=${pageInfo.condition}&keyword=${pageInfo.keyword}&hideNotice=${empty param.hideNotice ? false : param.hideNotice}"/>"> ${i} </a></span>	
					</c:if>
					<c:if test="${i != pageInfo.page}">
						<span><a class="page_navi" href="<c:url value="/board.do?page=${i}&pageSize=${pageInfo.pageSize}&condition=${pageInfo.condition}&keyword=${pageInfo.keyword}&hideNotice=${empty param.hideNotice ? false : param.hideNotice}"/>"> ${i} </a></span>
					</c:if>
				</c:forEach>
				
				<c:if test="${pageInfo.showNext}">
					<span><a class="page_navi" href="<c:url value="/board.do?page=${pageInfo.endPage+1}&pageSize=${pageInfo.pageSize}&condition=${pageInfo.condition}&keyword=${pageInfo.keyword}&hideNotice=${empty param.hideNotice ? false : param.hideNotice}"/>">&gt;</a></span>
				</c:if>
...
<script>
	...
	searchBtn.addEventListener("click", () => {
		console.log("click");
		form.method = "GET";
		form.action = "<c:url value="board.do"/>";
		form.submit();
	});
	...
	document.querySelector("#boardCnt").addEventListener("change", (e) => {

		if(${empty param.keyword}){			
			location.href="<c:url value="/board.do"/>?pageSize="+e.target.value;
		} else{

 			if(${empty param.page} || ${empty parma.pageSize}){
				location.href = "<c:url value="/board.do"/>?page=1" + "&pageSize=" + e.target.value + "&condition=${param.condition}" + "&keyword=${param.keyword}";	
			}else{
				location.href = "<c:url value="/board.do"/>?page=${param.page}" + "&pageSize=${e.target.value}" + "&condition=${param.condition}" + "&keyword=${param.keyword}";	
			} 

		}
	});
	
	document.querySelector("#hideNotice").addEventListener("change", (e) => {
		const hideCheckBoxState = e.target.checked;
		
		if(${empty param.keyword}){
			location.href = "<c:url value="/board.do"/>?hideNotice=" + hideCheckBoxState +"&pageSize=" + ${empty param.pageSize ? 10 : param.pageSize};
		}
		else{
			
			location.href = "<c:url value="/board.do"/>?hideNotice=" + hideCheckBoxState +"&pageSize=" + ${empty param.pageSize ? 10 : param.pageSize} + "&page=${empty param.page ? 1 : param.page}" + "&condition=${param.condition}" + "&keyword=${param.keyword}";
		}
	})
